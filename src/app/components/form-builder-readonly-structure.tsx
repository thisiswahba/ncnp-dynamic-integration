...
import { ChevronDown, ChevronUp, Eye, Edit2, Type, AlignLeft, List, Table2, Trash2, Save, ChevronRight, GripVertical, ExternalLink } from 'lucide-react';
...
// End-User Preview Component
interface EndUserPreviewProps {
  contentBlocks: ContentBlock[];
  title?: string;
  subtitle?: string;
  showTitle?: boolean;
...
function EndUserPreview({ contentBlocks, title, subtitle, showTitle = false }: EndUserPreviewProps) {
  const [isVisible, setIsVisible] = useState(true);

  if (!isVisible) return null;

  return (
    <div className="mt-4 border-2 border-blue-200 rounded-lg bg-blue-50/30 p-4">
      <div className="flex items-center justify-between mb-3">
        <h5 className="flex items-center gap-2 font-semibold text-foreground" style={{ fontSize: 'var(--text-sm)' }}>
          <Eye className="w-4 h-4 text-blue-600" />
          <span>👁 معاينة المستخدم النهائي</span>
        </h5>
        <button
      ...
      {contentBlocks.length === 0 ? (
        <div className="text-center py-6 text-muted-foreground bg-white rounded-lg border border-dashed border-border" style={{ fontSize: 'var(--text-sm)' }}>
                      ...
                      <th className="px-3 py-2 text-right border-b border-border bg-muted/50">
                        <button
                          onClick={() => {
                            ...
                            onUpdateContentBlock(criterionId, block.id, { ...block.data, columns: newColumns, rows: newRows }, indicatorId, practiceId, questionId);
                          }}
                        ...
                        <th key={colIndex} className="px-3 py-2 text-right border-b border-border">
                          <input
                            ...
                            onChange={(e) => {
                              ...
                              onUpdateContentBlock(criterionId, block.id, { ...block.data, columns: newColumns }, indicatorId, practiceId, questionId);
                            }}
...
export function FormBuilder({ onCancel, onSave, onPreview }: { onCancel: () => void; onSave: () => void; onPreview: () => void }) {
      ...
      contentBlocks: [
        {
          id: 'c1-block-1',
          type: 'heading',
          data: { text: 'نظرة عامة على الإطار التنظيمي', level: 2 }
        },
        {
          id: 'c1-block-2',
          type: 'text',
          data: { text: 'يركز هذا المعيار على تقييم مدى وجود إطار تنظيمي وهيكلي متين يدعم العمليات المالية في المؤسسة. يتضمن ذلك اللوائح والسياسات والإجراءات التي تحكم العمل المالي وتضمن الشفافية والمساءلة.' }
        },
        {
          id: 'c1-block-3',
          type: 'list',
          data: { 
            listType: 'bullet',
            items: [
              'التأكد من وجود لوائح مالية معتمدة ومحدثة',
              'قياس مدى الفصل بين الصلاحيات المالية والإدارية',
              'تقييم آليات الرقابة الداخلية المالية'
            ]
          }
        }
      ],
          ...
          contentBlocks: [
            {
              id: 'i1-1-block-1',
              type: 'text',
              data: { text: 'يهدف هذا المؤشر إلى قياس مدى توفر لوائح وسياسات مالية واضحة ومعتمدة تحكم العمليات المالية في المؤسسة.' }
            },
            {
              id: 'i1-1-block-2',
              type: 'heading',
              data: { text: 'الوثائق المطلوبة للمراجعة', level: 3 }
            },
            {
              id: 'i1-1-block-3',
              type: 'list',
              data: { 
                listType: 'number',
                items: [
                  'نسخة من اللائحة المالية المعتمدة',
                  'دليل السياسات المالية',
                  'قرار اعتماد اللوائح من الجهة المختصة',
              ...
              contentBlocks: [
                {
                  id: 'p1-1-1-block-1',
                  type: 'text',
                  data: { text: 'يجب أن تكون جميع السياسات المالية موثقة بشكل واضح ومعتمدة من الجهات المختصة. تشمل هذه السياسات:' }
                },
                {
                  id: 'p1-1-1-block-2',
                  type: 'table',
                  data: {
                    columns: [
                      { name: 'نوع السياسة', type: 'text' },
                      { name: 'الجهة المعتمدة', type: 'text' },
                      { name: 'تاريخ آخر تحديث', type: 'text' }
                    ],
                    rows: [
                      ['سياسة المشتريات', 'مجلس الإدارة', '2024'],
                      ['سياسة الصرف', 'المدير المالي', '2023'],
                      ['سياسة الاستثمار', 'اللجنة المالية', '2024']
                    ]
                  }
                }
              ],
                  ...
                  contentBlocks: [
                    {
                      id: 'q1-1-1-1-block-1',
                      type: 'text',
                      data: { text: 'يرجى تقديم قائمة بجميع السياسات المالية الموثقة والمعتمدة، مع ذكر تاريخ الاعتماد والجهة المعتمدة لكل سياسة.' }
                    },
                    {
                      id: 'q1-1-1-1-block-2',
                      type: 'heading',
                      data: { text: 'الأدلة المطلوبة', level: 3 }
                    },
                    {
                      id: 'q1-1-1-1-block-3',
                      type: 'list',
                      data: {
                        listType: 'bullet',
                        items: [
                          'نسخ من السياسات المالية المعتمدة',
                          'قرارات الاعتماد الرسمية',
                  ...
                  contentBlocks: [
                    {
                      id: 'q1-1-1-2-block-1',
                      type: 'text',
                      data: { text: 'يجب أن تكون السياسات المالية محدثة وتعكس أفضل الممارسات والتغييرات التنظيمية. يرجى ذكر تاريخ آخر تحديث لكل سياسة مالية.' }
                    }
                  ],
                  viewMode: 'edit'
                }
              ...
              contentBlocks: [
                {
                  id: 'p1-1-2-block-1',
                  type: 'heading',
                  data: { text: 'أهمية المراجعة الدورية', level: 3 }
                },
                {
                  id: 'p1-1-2-block-2',
                  type: 'text',
                  data: { text: 'تضمن المراجعة الدورية للسياسات المالية أن تبقى متوافقة مع التغييرات التنظيمية والتشريعية وأفضل الممارسات في مجال الإدارة المالية.' }
                }
              ],
              viewMode: 'edit',
              questions: [
                  ...
                  contentBlocks: [
                    {
                      id: 'q1-1-2-1-block-1',
                      type: 'text',
                      data: { text: 'يرجى تحديد الفترة الزمنية بين كل مراجعة والأخرى، والجهة المسؤولة عن المراجعة.' }
                    }
                  ],
                  viewMode: 'edit'
                }
          ...
          contentBlocks: [
            {
              id: 'i1-2-block-1',
              type: 'text',
              data: { text: 'يعد الفصل بين الصلاحيات من أهم مبادئ الحوكمة المالية الرشيدة، حيث يمنع تركز السلطة ويقلل من مخاطر التلاعب أو الاحتيال المالي.' }
            }
          ],
          viewMode: 'edit',
          practices: [
            {
              ...
              questions: [
                {
                  id: '1-2-1-1',
                  title: 'هل يوجد مصفو��ة صلاحيات مالية محددة؟',
                  isExpanded: false,
                  contentBlocks: [],
                  viewMode: 'edit'
                }
      ...
      contentBlocks: [
        {
          id: 'c2-block-1',
          type: 'heading',
          data: { text: 'إدارة المخاطر المالية', level: 2 }
        },
        {
          id: 'c2-block-2',
          type: 'text',
          data: { text: 'تعد إدارة المخاطر المالية عنصراً حاسماً في الحوكمة المالية الفعالة. يركز هذا المعيار على قياس مدى قدرة المؤسسة على تحديد وتقييم ومعالجة المخاطر المالية بشكل استباقي.' }
        },
        {
          id: 'c2-block-3',
          type: 'list',
          data: { 
            ...
            items: [
              ...
              'تقييم احتمالية وأثر كل خطر',
              'وضع خطط معالجة فعالة',
              'مراقبة ومراجعة المخاطر بشكل دوري'
            ]
          }
        }
      ],
          ...
          contentBlocks: [
            {
              id: 'i2-1-block-1',
              type: 'heading',
              data: { text: 'أنواع المخاطر المالية', level: 3 }
            },
            {
              id: 'i2-1-block-2',
              type: 'table',
              data: {
                columns: [
                  { name: 'نوع الخطر', type: 'text' },
                  { name: 'الوصف', type: 'text' },
                  { name: 'مستوى الأولوية', type: 'text' }
                ],
                rows: [
                  ['مخاطر السيولة', 'عدم القدرة على الوفاء بالالتزامات', 'عالي'],
                  ['مخاطر الائتمان', 'عدم سداد المدينين', 'متوسط'],
                  ['مخاطر السوق', 'تقلبات الأسعار والعملات', 'عالي']
                ]
              }
            }
          ],
              ...
              contentBlocks: [
                {
                  id: 'p2-1-1-block-1',
                  type: 'text',
                  data: { text: 'سجل المخاطر هو أداة حيوية لتتبع وإدارة جميع المخاطر المالية التي قد تواجه المؤسسة. يجب أن يتضمن السجل معلومات تفصيلية عن كل خطر.' }
                }
              ],
              viewMode: 'edit',
              questions: [
  ...
  // Question Details Side Sheet State
  ...
  const handleQuestionClick = (
    ...
    const practice = indicator?.practices.find(p => p.id === practiceId);
  ...
  const handleQuestionSave = (data: { score: number; reason: string; attachment?: File }) => {
    ...
    // Update the question with override data
    setCriteria(prev => prev.map(crit =>
      crit.id === selectedQuestion.criterionId
        ? {
            ...crit,
            indicators: crit.indicators.map(ind =>
              ind.id === selectedQuestion.indicatorId
                ? {
                    ...ind,
                    practices: ind.practices.map(prac =>
                      prac.id === selectedQuestion.practiceId
                        ? {
                            ...prac,
  ...
  return (
    <DndProvider backend={HTML5Backend}>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-muted-foreground mt-1" style={{ fontSize: 'var(--text-sm)' }}>
                  🔒 الهيكل الهرمي للمعايير مصدره خارجي (للقراءة فقط) • يمكن تعديل المحتوى فقط
                </p>
              </div>
          ...
          <div className="space-y-6">
            {criteria.map((criterion) => (
              <div key={criterion.id} className="bg-white rounded-lg border-2 border-border shadow-sm">
                {/* LEVEL 2: STANDARD (معيار) - READ ONLY TITLE */}
                <div className="p-6">
                    ...
                    <>
                      ...
                      <div className="rounded-lg p-4 mb-6" style={{ backgroundColor: '#FFF8E1', border: '1px solid #FFE082' }}>
                        <div className="flex items-center gap-2 mb-3 justify-between">
                          ...
                          <h4 className="font-semibold text-foreground" style={{ fontSize: 'var(--text-base)' }}>
                            محتوى المعيار
                          </h4>
                        </div>
                        ...
                        {criterion.contentBlocks.length > 0 ? (
                          ...
                          <div className="text-center py-4 text-muted-foreground bg-white/50 rounded" style={{ fontSize: 'var(--text-sm)' }}>
                            لم يتم إضافة محتوى بعد
                          </div>
                        ) : (
                          <div className="text-center py-4 text-muted-foreground bg-white/50 rounded border border-dashed border-border" style={{ fontSize: 'var(--text-sm)' }}>
                            لم يتم إضافة محتوى بعد
                          </div>
                        )}
                        ...
                        {/* End-User Preview for Criterion */}
                        <EndUserPreview 
                          contentBlocks={criterion.contentBlocks}
                          title={criterion.title}
                          showTitle={true}
                        />
                      </div>

                      {/* LEVEL 3: INDICATORS (مؤشرات) - READ ONLY TITLES */}
                                ...
                                <>
                                  ...
                                  <div className="rounded-lg p-3 mb-4 bg-white border border-blue-200">
                                    ...
                                    {indicator.contentBlocks.length > 0 ? (
                                      ...
                                      <div className="text-center py-3 text-muted-foreground bg-muted/10 rounded" style={{ fontSize: 'var(--text-xs)' }}>
                                        لم يتم إضافة محتوى بعد
                                      </div>
                                    ) : (
                                      <div className="text-center py-3 text-muted-foreground bg-muted/10 rounded border border-dashed border-border" style={{ fontSize: 'var(--text-xs)' }}>
                                        لم يتم إضافة محتوى بعد
                                      </div>
                                    )}
                                    ...
                                    {/* End-User Preview for Indicator */}
                                    <EndUserPreview 
                                      contentBlocks={indicator.contentBlocks}
                                  ...
                                  {/* LEVEL 4: PRACTICES (ممارسات) - READ ONLY TITLES */}
                                      ...
                                      indicator.practices.map((practice) => (
                                        <div key={practice.id} className="border-2 border-green-200 rounded-lg bg-green-50/30 p-3">
                                          ...
                                          {practice.isExpanded && (
                                            <>
                                              {/* Practice Content */}
                                              <div className="rounded-lg p-2 mb-3 bg-white border border-green-200">
                                                <div className="flex items-center gap-1 mb-2 justify-between">
                                                  <button
                                                    onClick={() => toggleViewMode(criterion.id, indicator.id, practice.id)}
                                                ...
                                                {practice.contentBlocks.length > 0 ? (
                                                  ...
                                                  <div className="text-center py-2 text-muted-foreground bg-muted/10 rounded text-xs">
                                                    لم يتم إضافة محتوى بعد
                                                  </div>
                                                ) : (
                                                  <div className="text-center py-2 text-muted-foreground bg-muted/10 rounded border border-dashed border-border text-xs">
                                                    لم يتم إضافة محتوى بعد
                                                  </div>
                                                )}
                                                ...
                                                {practice.viewMode === 'edit' && (
                                                  <div className="flex flex-wrap gap-1">
                                                    <button
                                                      onClick={() => addContentBlock(criterion.id, 'heading', indicator.id, practice.id)}
                                                    ...
                                                    <button
                                                      onClick={() => addContentBlock(criterion.id, 'text', indicator.id, practice.id)}
                                                      className="flex items-center gap-1 px-2 py-1 border border-border rounded hover:bg-muted transition-colors text-foreground bg-white text-xs"
                                                    >
                                                      <AlignLeft className="w-3 h-3" />
                                                      <span>نص</span>
                                                    ...
                                                    <button
                                                      onClick={() => addContentBlock(criterion.id, 'list', indicator.id, practice.id)}
                                                      className="flex items-center gap-1 px-2 py-1 border border-border rounded hover:bg-muted transition-colors text-foreground bg-white text-xs"
                                                    >
                                                      <List className="w-3 h-3" />
                                                      <span>قائمة</span>
                                                    ...
                                                    <button
                                                      onClick={() => addContentBlock(criterion.id, 'table', indicator.id, practice.id)}
                                                      className="flex items-center gap-1 px-2 py-1 border border-border rounded hover:bg-muted transition-colors text-foreground bg-white text-xs"
                                                    >
                                                      <Table2 className="w-3 h-3" />
                                                      <span>جدول</span>
                                                ...
                                                {/* End-User Preview for Practice */}
                                                <EndUserPreview 
                                                  contentBlocks={practice.contentBlocks}
                                              ...
                                              {/* LEVEL 5: QUESTIONS (أسئلة) - READ ONLY TITLES */}
      ...
      <QuestionDetailsSidesheet
        ...
        linkedContent={selectedQuestion?.question.linkedContent || []}
        aiScore={selectedQuestion?.question.aiScore}
        currentScore={selectedQuestion?.question.manualScore}
        overrideReason={selectedQuestion?.question.overrideReason}
        isOverridden={selectedQuestion?.question.isOverridden}
